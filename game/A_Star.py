...
def A_Star(screen, survivor):
    ...
    def loop():
        ...
        for node2 in open_list:
            G(node2)
            F(node2)

            pygame.draw.line(screen, [255, 0, 0],
                             [node2.parent.x + half, node2.parent.y + half],
                             [node2.x + half, node2.y + half])

        loop()

    for zombie in Zombie.List:
        open_list = []  # tiles to check
        closed_list = []  # tiles already checked
